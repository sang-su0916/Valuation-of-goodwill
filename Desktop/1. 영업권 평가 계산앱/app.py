...
try:
    ...
    # 오류 메시지를 로깅만 하고 프로그램은 계속 실행
    print("dotenv 패키지를 찾을 수 없습니다. 환경 변수를 직접 설정하세요.")
    ...
    try:
        ...
        if revenue <= 0:
            ...
            if api_key:
                ...
                # 더 정교한 성장 모델 적용
                revenues = [revenue * (1 + max(0.1, growth_rate)/100 * math.exp(-0.1 * year) * industry_factor)**year for year in years]
                profit_margin = operating_profit/revenue * risk_factor if revenue > 0 else 0
                profits = [rev * profit_margin for rev in revenues]
                
                # 전문가용 할인율 조정
                adjusted_discount_rate = max(0.1, discount_rate * risk_factor)  # 할인율이 0이 되지 않도록 방지
            else:
                # 기본 모드 계산 (단순한 모델)
            ...
            summary_data = {
                ...
                "내용": [
                    ...
                    valuation_type if valuation_type else "미입력",
                    f"{revenue:,.0f} 백만원",
            ...
            if api_key:
                try:
                    # PDF 보고서 생성
                    report_data = {
                        'company_name': company_name if company_name else "미입력",
                        'evaluation_date': evaluation_date.strftime("%Y-%m-%d") if evaluation_date else "미입력",
